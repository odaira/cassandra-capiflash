...
public class Keyspace {
	...
	/**
	 * Removes every SSTable in the directory from the appropriate DataTracker's
	 * view.
	 * 
	 * @param directory
	 ...
	 * column families if columnFamily is null with a given timestamp
	 *
	 * @param snapshotName
	 *            the tag associated with the name of the snapshot. This value
	 *            may not be null
	 ...
	 *
	 * @param snapshotName
	 *            the user supplied snapshot name. It empty or null, all the
	...
	private void createReplicationStrategy(KSMetaData ksm) {
		replicationStrategy = AbstractReplicationStrategy.createReplicationStrategy(ksm.name, ksm.strategyClass,
	...
	public void initCf(UUID cfId, String cfName, boolean loadSSTables) {
		...
		if (cfs == null) {
			// CFS being created for the first time, either on server startup or
			// new CF being added.
			...
			// CFS mbean instantiation will error out before we hit this, but in
			// case that changes...
			if (oldCfs != null)
				throw new IllegalStateException("added multiple mappings for cf id " + cfId);
		} else {
			// re-initializing an existing CF. This will happen if you cleared
			// the schema
	 ...
	 *
	 * @param mutation
	 *            the row to write. Must not be modified after calling apply,
	 *            since commitlog append may happen concurrently, depending on
	 *            the CL Executor type.
	 * @param writeCommitLog
	 *            false to disable commitlog append entirely
	 * @param updateIndexes
	 *            false to disable index updates (used by CollationController
	 *            "defragmenting")
	 */
	public void apply(Mutation mutation, boolean writeCommitLog, boolean updateIndexes) {
		try (OpOrder.Group opGroup = writeOrder.start()) {
			...
			for (ColumnFamily cf : mutation.getColumnFamilies()) {
				...
				SecondaryIndexManager.Updater updater = updateIndexes ? cfs.indexManager.updaterFor(key, cf, opGroup)
						: SecondaryIndexManager.nullUpdater;
				cfs.apply(key, cf, updater, opGroup, replayPosition);
			}
		} catch (CommitlogOutofSpaceException outOfSpace) {
			logger.error(x.incrementAndGet() + " out of space exception thrown!!!");
			CommitLogHelper.instance.await();// noop for default commitlog
	 ...
	 *            row to index
	 * @param cfs
	 *            ColumnFamily to index row in
	 * @param idxNames
	 *            columns to index, in comparator order
	 */
