...
public class ColumnFamilyStore implements ColumnFamilyStoreMBean {
	...
	public void reload() {
		// metadata object has been mutated directly. make all the members jibe
		// with new settings.
	...
	private void maybeReloadCompactionStrategy() {
		...
		// synchronize vs runWithCompactionsDisabled calling pause/resume.
		// otherwise, letting old compactions
			...
			WrappedRunnable runnable = new WrappedRunnable() {
				protected void runMayThrow() throws Exception {
					if (getMemtableThreadSafe().isExpired()) {
						// if memtable is already expired but didn't flush
						// because it's empty,
						// then schedule another flush.
						if (isClean())
							scheduleFlush();
						else
							forceFlush(); // scheduleFlush() will be called by
											// the constructor of the new
											// memtable.
					}
				}
			};
	...
	public static Runnable getBackgroundCompactionTaskSubmitter() {
		return new Runnable() {
			public void run() {
				...
				while (!submitted.isEmpty() && CompactionManager.instance
						.getActiveCompactions() < CompactionManager.instance.getMaximumCompactorThreads()) {
	...
	private ColumnFamilyStore(Keyspace keyspace, String columnFamilyName, IPartitioner partitioner, int generation,
			CFMetaData metadata, Directories directories, boolean loadSSTables) {
		...
		mbeanName = "org.apache.cassandra.db:type=" + type + ",keyspace=" + this.keyspace.getName() + ",columnfamily="
				+ name;
		try {
	 ...
	 * 
	 * @param directory
	 *            the unreadable directory, possibly with SSTables in it, but
	 *            not necessarily.
	 */
	...
	public static ColumnFamilyStore createColumnFamilyStore(Keyspace keyspace, String columnFamily,
			boolean loadSSTables) {
		return createColumnFamilyStore(keyspace, columnFamily, StorageService.getPartitioner(),
				Schema.instance.getCFMetaData(keyspace.getName(), columnFamily), loadSSTables);
	}

	public static ColumnFamilyStore createColumnFamilyStore(Keyspace keyspace, String columnFamily,
			IPartitioner partitioner, CFMetaData metadata) {
		return createColumnFamilyStore(keyspace, columnFamily, partitioner, metadata, true);
	}

	private static synchronized ColumnFamilyStore createColumnFamilyStore(Keyspace keyspace, String columnFamily,
			IPartitioner partitioner, CFMetaData metadata, boolean loadSSTables) {
		...
		for (Map.Entry<Descriptor, Set<Component>> entry : lister.list().entrySet()) {
			...
			if (!desc.isCompatible())
				throw new RuntimeException(String.format(
						"Incompatible SSTable found.  Current version %s is unable to read file: %s.  Please run upgradesstables.",
	...
	/**
	 * Removes unnecessary files from the cf directory at startup: these include
	 * temp files, orphans, zero-length files and compacted sstables. Files that
	 * cannot be recognized will be ignored.
	 */
	public static void scrubDataDirectories(String keyspaceName, String columnFamily) {
		...
		for (File dir : directories.getCFDirectories()) {
			...
			if (lockfiles == null || lockfiles.length == 0)
				continue;
			logger.info("Removing SSTables from failed streaming session. Found {} files to cleanup.",
					lockfiles.length);
	...
	/**
	 * Replacing compacted sstables is atomic as far as observers of DataTracker
	 * are concerned, but not on the filesystem: first the new sstables are
	 * renamed to "live" status (i.e., the tmp marker is removed), then their
	 * ancestors are removed.
	 *
	 * If an unclean shutdown happens at the right time, we can thus end up with
	 * both the new ones and their ancestors "live" in the system. This is
	 * harmless for normal data, but for counters it can cause overcounts.
	 *
	 * To prevent this, we record sstables being compacted in the system
	 * keyspace. If we find unfinished compactions, we remove the new ones
	 * (since those may be incomplete -- under LCS, we may create multiple
	 * sstables from any given ancestor).
	 */
	public static void removeUnfinishedCompactionLeftovers(String keyspace, String columnfamily,
		...
		if (!allGenerations.containsAll(unfinishedGenerations)) {
			...
			logger.debug("Unfinished compactions of {}.{} reference missing sstables of generations {}", keyspace,
					columnfamily, missingGenerations);
		}
	...
	public void initRowCache() {
		...
		if (cachedRowsRead > 0)
			logger.info("completed loading ({} ms; {} keys) row cache for {}.{}",
					TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start), cachedRowsRead, keyspace.getName(), name);
	}
	...
	public synchronized void loadNewSSTables() {
		...
		for (Map.Entry<Descriptor, Set<Component>> entry : lister.list().entrySet()) {
			...
			if (!descriptor.isCompatible())
				throw new RuntimeException(
						String.format("Can't open incompatible SSTable! Current version %s, found file: %s",
								Descriptor.Version.CURRENT, descriptor));

			// force foreign sstables to level 0
			try {
				if (new File(descriptor.filenameFor(Component.STATS)).exists()) {
					Pair<SSTableMetadata, Set<Integer>> oldMetadata = SSTableMetadata.serializer
			...
			// Increment the generation until we find a filename that doesn't
			// exist. This is needed because the new
			// SSTables that are being loaded might already use these generation
			// numbers.
			Descriptor newDescriptor;
			do {
				newDescriptor = new Descriptor(descriptor.version, descriptor.directory, descriptor.ksname,
						descriptor.cfname, fileIndexGenerator.incrementAndGet(), false);
		...
		logger.info("Loading new SSTables and building secondary indexes for " + keyspace.getName() + "/" + name + ": "
				+ newSSTables);
	...
	private String getTempSSTablePath(File directory, Descriptor.Version version) {
		Descriptor desc = new Descriptor(version, directory, keyspace.getName(), name,
				fileIndexGenerator.incrementAndGet(), true);
	...
	public Future<?> switchMemtable(final boolean writeCommitLog, boolean forceSwitch) {
		 ...
		 * and all ongoing updates to memtables have completed. We can get the
		 * tail of the log and use it as the starting position for log replay on
		 * recovery.
		 *
		 * This is why we Keyspace.switchLock needs to be global instead of
		 * per-Keyspace: we need to schedule discardCompletedSegments calls in
		 * the same order as their contexts (commitlog position) were read, even
		 * though the flush executor is multithreaded.
		 */
		...
		try {
			...
			// here during index build even if it is not
			for (ColumnFamilyStore cfs : concatWithIndexes()) {
			...
			// when all the memtables have been written, including for indexes,
	...
	private boolean isClean() {
		// during index build, 2ary index memtables can be dirty even if parent
		// is not. if so,
	...
	/**
	 * @return a future, with a guarantee that any data inserted prior to the
	 *         forceFlush() call is fully flushed by the time future.get()
	...
	/**
	 * Insert/Update the column family for this key. Caller is responsible for
	 * acquiring Keyspace.switchLock param @ lock - lock that needs to be used.
	 * param @ key - key for update/insert param @ columnFamily - columnFamily
	 * changes
	 */
	 ...
	 * are any columns or tombstones left, null otherwise.
	 * 
	 * @param gcBefore
	 *            a timestamp (in seconds); tombstones with a localDeletionTime
	 *            before this will be purged
	 */
	 ...
	 * 
	 * @return an updated `cf` if any columns or tombstones remain, null
	 *         otherwise
	 */
	 ...
	 * supercolumns, and columnfamilies until they have been deleted for at
	 * least GC_GRACE_IN_SECONDS. But, we do not need to preserve their
	 * contents; just the object itself as a "tombstone" that can be used to
	 * repair other replicas that do not know about the deletion.
	 */
	...
	/**
	 * Removes only per-cell tombstones, cells that are shadowed by a row-level
	 * or range tombstone, or columns that have been dropped from the schema
	 * (for CQL3 tables only).
	 * 
	...
	public static long removeDeletedColumnsOnly(ColumnFamily cf, int gcBefore, SecondaryIndexManager.Updater indexer) {
		...
		while (iter.hasNext()) {
			...
			// only)
			if (c.getLocalDeletionTime() < gcBefore || tester.isDeleted(c)
					|| (hasDroppedColumns && isDroppedColumn(c, cf.metadata()))) {
				iter.remove();
				indexer.remove(c);
	 ...
	 *         sstables, not including itself. (The given sstables may or may
	...
	public Set<SSTableReader> getOverlappingSSTables(Collection<SSTableReader> sstables) {
		...
		for (SSTableReader sstable : sstables) {
			Set<SSTableReader> overlaps = ImmutableSet
					.copyOf(tree.search(Interval.<RowPosition, SSTableReader> create(sstable.first, sstable.last)));
	 ...
	 * file via bootstrap. This information is cached in the ColumnFamilyStore.
	 * This is useful for reads because the ColumnFamilyStore first looks in the
	 * in-memory store and the into the disk to find the key. If invoked during
	 * recoveryMode the onMemtableFlush() need not be invoked.
	 *
	 ...
	 *
	 * If operation type is {@code CLEANUP} and we're not dealing with an index
	 * sstable, then we calculate expected file size with checking token range
	 * to be eliminated.
	 *
	 * Otherwise, we just add up all the files' size, which is the worst case
	 * file size for compaction of all the list of files given.
	 *
	 * @param sstables
	 *            SSTables to calculate expected compacted file size
	 * @param operation
	 *            Operation type
	...
	public void scrub(boolean disableSnapshot, boolean skipCorrupted, boolean checkData)
	...
	public void replaceCompactedSSTables(Collection<SSTableReader> sstables, Collection<SSTableReader> replacements,
	...
	/**
	 * @return the live size of all the memtables (the current active one and
	 *         pending flush).
	 */
	...
	/**
	 * @return the size of all the memtables, including the pending flush ones
	 *         and 2i memtables, if any.
	 */
	...
	public ColumnFamily getColumnFamily(DecoratedKey key, ByteBuffer start, ByteBuffer finish, boolean reversed,
			int limit, long timestamp) {
		return getColumnFamily(QueryFilter.getSliceFilter(key, name, start, finish, reversed, limit, timestamp));
	}

	/**
	 * fetch the row given by filter.key if it is in the cache; if not, read it
	 * from disk and cache it
	 * 
	 * @param cfId
	 *            the column family to read the row from
	 * @param filter
	 *            the columns being queried. Note that we still cache entire
	 *            rows, but if a row is uncached and we race to cache it, only
	 *            the winner will read the entire row
	 * @return the entire row for filter.key, if present in the cache (or we can
	 *         cache it), or just the column specified by filter otherwise
	 */
	...
	/**
	 * Filter a cached row, which will not be modified by the filter, but may be
	 * modified by throwing out tombstones that are no longer relevant. The
	 * returned column family won't be thread safe.
	 */
	...
	/**
	 * Get the current view and acquires references on all its sstables. This is
	 * a bit tricky because we must ensure that between the time we get the
	 * current view and the time we acquire the references the set of sstables
	 * hasn't changed. Otherwise we could get a view for which an sstable have
	...
	/**
	 * Get the current sstables, acquiring references on all of them. The caller
	 * is in charge of releasing the references on the sstables.
	 *
	...
	private ViewFragment markReferenced(Function<DataTracker.View, List<SSTableReader>> filter) {
		...
		while (true) {
			...
			if (null != prevSstables) {
				// we're trying again so verify we're acquiring something
				// different
		...
		return new ViewFragment(sstables,
				Iterables.concat(Collections.singleton(view.memtable), view.memtablesPendingFlush));
	}

	/**
	 * @return a ViewFragment containing the sstables and memtables that may
	 *         need to be merged for the given @param key, according to the
	 *         interval tree
	 */
	...
	/**
	 * @return a ViewFragment containing the sstables and memtables that may
	 *         need to be merged for rows within @param rowBounds, inclusive,
	...
	/**
	 * @return a ViewFragment containing the sstables and memtables that may
	 *         need to be merged for rows for all of @param rowBoundsCollection,
	 *         inclusive, according to the interval tree.
	 */
	 ...
	 *
	 * @param range
	 *            The range of keys and columns within those keys to fetch
	 */
	private AbstractScanIterator getSequentialIterator(final DataRange range, long now) {
		...
		try {
			final CloseableIterator<Row> iterator = RowIteratorFactory.getIterator(view.memtables, view.sstables, range,
					this, now);
			...
			// In case getIterator() throws, otherwise the iteror close method
	...
	@VisibleForTesting
	public List<Row> getRangeSlice(final AbstractBounds<RowPosition> range, List<IndexExpression> rowFilter,
			IDiskAtomFilter columnFilter, int maxResults) {
		return getRangeSlice(range, rowFilter, columnFilter, maxResults, System.currentTimeMillis());
	}

	public List<Row> getRangeSlice(final AbstractBounds<RowPosition> range, List<IndexExpression> rowFilter,
			IDiskAtomFilter columnFilter, int maxResults, long now) {
		return getRangeSlice(makeExtendedFilter(range, columnFilter, rowFilter, maxResults, false, false, now));
	}

	/**
	 * Allows generic range paging with the slice column filter. Typically,
	 * suppose we have rows A, B, C ... Z having each some columns in [1, 100].
	 * And suppose we want to page through the query that for all rows returns
	 * the columns within [25, 75]. For that, we need to be able to do a range
	 * slice starting at (row r, column c) and ending at (row Z, column 75),
	 * *but* that only return columns in [25, 75]. That is what this method
	 * allows. The columnRange is the "window" of columns we are interested in
	 * each row, and columnStart (resp. columnEnd) is the start (resp. end) for
	 * the first (resp. last) requested row.
	 */
	public ExtendedFilter makeExtendedFilter(AbstractBounds<RowPosition> keyRange, SliceQueryFilter columnRange,
			ByteBuffer columnStart, ByteBuffer columnStop, List<IndexExpression> rowFilter, int maxResults,
			boolean countCQL3Rows, long now) {
	...
	public List<Row> getRangeSlice(AbstractBounds<RowPosition> range, List<IndexExpression> rowFilter,
			IDiskAtomFilter columnFilter, int maxResults, long now, boolean countCQL3Rows, boolean isPaging) {
		return getRangeSlice(
				makeExtendedFilter(range, columnFilter, rowFilter, maxResults, countCQL3Rows, isPaging, now));
	}

	public ExtendedFilter makeExtendedFilter(AbstractBounds<RowPosition> range, IDiskAtomFilter columnFilter,
			List<IndexExpression> rowFilter, int maxResults, boolean countCQL3Rows, boolean isPaging, long timestamp) {
		DataRange dataRange;
		if (isPaging) {
			...
			// partitions
			SliceQueryFilter newFilter = new SliceQueryFilter(ColumnSlice.ALL_COLUMNS_ARRAY, sfilter.isReversed(),
					sfilter.count);
	...
	@VisibleForTesting
	public List<Row> search(AbstractBounds<RowPosition> range, List<IndexExpression> clause, IDiskAtomFilter dataFilter,
			int maxResults) {
		return search(range, clause, dataFilter, maxResults, System.currentTimeMillis());
	}

	public List<Row> search(AbstractBounds<RowPosition> range, List<IndexExpression> clause, IDiskAtomFilter dataFilter,
			int maxResults, long now) {
		return search(makeExtendedFilter(range, dataFilter, clause, maxResults, false, false, now));
	}

	public List<Row> search(ExtendedFilter filter) {
		Tracing.trace("Executing indexed scan for {}",
				filter.dataRange.keyRange().getString(metadata.getKeyValidator()));
	...
	public List<Row> filter(AbstractScanIterator rowIterator, ExtendedFilter filter) {
		...
		try {
			while (rowIterator.hasNext() && matched < filter.maxRows() && columnsCount < filter.maxColumns()) {
				// get the raw columns requested, and additional columns for the
				...
				if (rowIterator.needsFiltering()) {
					...
					if (extraFilter != null) {
						ColumnFamily cf = filter.cfs
								.getColumnFamily(new QueryFilter(rawRow.key, name, extraFilter, filter.timestamp));
						if (cf != null)
	...
	public List<SSTableReader> getSnapshotSSTableReader(String tag) throws IOException {
		...
		try {
			for (Map.Entry<Descriptor, Set<Component>> entries : snapshots.entrySet()) {
				...
				// to avoid opening new sstables. If it fails, use the snapshot
				// reference instead.
	 ...
	 *
	 * @param snapshotName
	 *            the user supplied snapshot name. If left empty, all the
	...
	/**
	 * @return the cached row for @param key if it is already present in the
	 *         cache. That is, unlike getThroughCache, it will not readAndCache
	 *         the row if it is not present, nor are these calls counted in
	 *         cache statistics.
	 *
	 *         Note that this WILL cause deserialization of a SerializingCache
	 *         row, so if all you need to know is whether a row is present or
	 *         not, use containsCachedRow instead.
	 */
	...
	public boolean containsCachedRow(DecoratedKey key) {
		return CacheService.instance.rowCache.getCapacity() != 0
				&& CacheService.instance.rowCache.containsKey(new RowCacheKey(metadata.cfId, key));
	}
	...
	public static Iterable<ColumnFamilyStore> all() {
		List<Iterable<ColumnFamilyStore>> stores = new ArrayList<Iterable<ColumnFamilyStore>>(
	 ...
	 * memtables, nor for thread safety. All we do is wipe the sstable
	 * containers clean, while leaving the actual data files present on disk.
	 * (This allows tests to easily call loadNewSSTables on them.)
	 */
			...
			try {
				...
				for (ColumnFamilyStore cfs : selfWithIndexes) {
					if (!cfs.getDataTracker().getCompacting().isEmpty()) {
						logger.warn(
								"Unable to cancel in-progress compactions for {}.  Probably there is an unusually large row in progress somewhere.  It is also possible that buggy code left some sstables compacting after it was done with them",
								metadata.cfName);
					}
				}
	...
	public Iterable<SSTableReader> markAllCompacting() {
		Callable<Iterable<SSTableReader>> callable = new Callable<Iterable<SSTableReader>>() {
			public Iterable<SSTableReader> call() throws Exception {
				...
				Iterable<SSTableReader> sstables = Lists
						.newArrayList(AbstractCompactionStrategy.filterSuspectSSTables(getSSTables()));
	...
	public void disableAutoCompaction() {
		// we don't use CompactionStrategy.pause since we don't want users
		// flipping that on and off
	...
	/*
	 * JMX getters and setters for the Default<T>s. - get/set
	 * minCompactionThreshold - get/set maxCompactionThreshold - get memsize -
	 * get memops - get/set memtime
	 */
	...
	private void validateCompactionThresholds(int minThreshold, int maxThreshold) {
		if (minThreshold > maxThreshold)
			throw new RuntimeException(
					String.format("The min_compaction_threshold cannot be larger than the max_compaction_threshold. "
							+ "Min is '%d', Max is '%d'.", minThreshold, maxThreshold));

		if (maxThreshold == 0 || minThreshold == 0)
			throw new RuntimeException(
					"Disabling compaction by setting min_compaction_threshold or max_compaction_threshold to 0 "
	...
	public int getUnleveledSSTables() {
		return this.compactionStrategy instanceof LeveledCompactionStrategy
				? ((LeveledCompactionStrategy) this.compactionStrategy).getLevelSize(0) : 0;
	}

	public int[] getSSTableCountPerLevel() {
		return compactionStrategy instanceof LeveledCompactionStrategy
				? ((LeveledCompactionStrategy) compactionStrategy).getAllLevelSize() : null;
	}
	...
	private boolean isRowCacheEnabled() {
		return !(metadata.getCaching() == Caching.NONE || metadata.getCaching() == Caching.KEYS_ONLY
	 ...
	 *
	 * @param truncatedAt
	 *            The timestamp of the truncation (all SSTables before that
	 *            timestamp are going be marked as compacted)
	 *
