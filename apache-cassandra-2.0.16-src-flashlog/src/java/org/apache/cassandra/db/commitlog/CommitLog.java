...
public class CommitLog implements CommitLogMBean, ICommitLog {
	...
	public static final int END_OF_SEGMENT_MARKER = 0; // this is written out at
														// the end of a segment
	public static final int END_OF_SEGMENT_MARKER_SIZE = 4; // number of bytes
															// of ^^^
	...
	/**
	 * Perform recovery on commit logs located in the directory specified by the
	 * config file.
	 *
	 ...
	 *
	 * @param clogs
	 *            the list of commit log files to replay
	 * @return the number of mutations replayed
	 */
	 ...
	 *         ready, all commitlog tasks enqueued prior to the getContext call
	 *         will be complete (i.e., appended to the log)
	 */
	 ...
	 * Used by tests.
	 *
	 * @return the number of active segments (segments with unflushed data in
	 *         them)
	 */
	 ...
	 *
	 * @param rm
	 *            the RowMutation to add to the log
	 */
	 ...
	 * column family according to the position given. Discards any commit log
	...
	/**
	 * @return the total size occupied by commitlo segments expressed in bytes.
	 *         (used by MBean)
	 */
	...
	// TODO this should be a Runnable since it doesn't actually return anything,
	...
	class LogRecordAdder implements Callable, Runnable {
		...
		public void run() {
			...
			if (!activeSegment.hasCapacityFor(totalSize)) {
				...
				activateNextSegment();
				// Now we can run the user defined command just before switching
